...
public interface RecipeMapper {
	@Select("SELECT no,title,poster,rownum " + "FROM (SELECT no,title,poster " + "FROM recipe ORDER BY hit DESC) "
			+ "WHERE rownum<=12")
	public List<RecipeVO> recipeHome12();

	@Select("SELECT cno,chef,poster,rownum " + "FROM (SELECT cno,chef,poster " + "FROM chef ORDER BY mem_cont7 DESC) "
			+ "WHERE rownum<=12")
	...
	@Select("SELECT CEIL(COUNT(*)/20.0) FROM recipe " + "WHERE no IN(SELECT no FROM recipe "
			+ "intersect SELECT no FROM recipeDetail)")
	...
	// 쉐프별 레시피
	@Select("SELECT no,title,poster,chef,num " + "FROM (SELECT no,title,poster,chef,rownum as num "
			+ "FROM (SELECT no,title,poster,chef " + "FROM recipe WHERE chef=(SELECT chef FROM chef WHERE cno=#{cno}))) "
			+ "WHERE num BETWEEN #{start} AND #{end}")
	public List<RecipeVO> chefDetailData(Map map);

	@Select("SELECT CEIL(COUNT(*)/20.0) FROM recipe " + "WHERE chef=(SELECT chef FROM chef WHERE cno=#{cno})")
	public int chefDetailTotalPage(int cno);

	@Select("SELECT no,title,poster,chef,num " + "FROM (SELECT no,title,poster,chef,rownum as num "
			+ "FROM (SELECT no,title,poster,chef " + "FROM recipe WHERE chef=(SELECT chef FROM chef WHERE cno=#{cno}) "
			+ "AND title LIKE '%'||#{ss}||'%')) " + "WHERE num BETWEEN #{start} AND #{end}")
	...
	/*
	 * NO NUMBER POSTER NOT NULL VARCHAR2(300) TITLE NOT NULL VARCHAR2(1000) CHEF
	 * NOT NULL VARCHAR2(200) CHEF_POSTER VARCHAR2(300) CHEF_PROFILE VARCHAR2(1000)
	 * INFO1 VARCHAR2(100) INFO2 VARCHAR2(100) INFO3 VARCHAR2(100) CONTENT
	 * VARCHAR2(4000) FOODMAKE CLOB STUFF CLOB
	 */
	@Select("SELECT * FROM recipeDetail " + "WHERE no=#{no}")
	public RecipeDetailVO recipeDetailData(int no);

	@Select("SELECT goods_poster,goods_name,goods_price " + "FROM goods_all "
