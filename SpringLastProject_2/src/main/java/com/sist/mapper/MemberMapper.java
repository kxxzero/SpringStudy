...
public interface MemberMapper {
	...
	@Insert("INSERT INTO projectMember(userid,username,userpwd,sex,birthday,email,post,"
			+ "addr1,addr2,phone,content) VALUES(#{userId},#{userName}," + "#{userPwd},#{sex},#{birthday},#{email},#{post},"
	...
	// memberIdCount() 재사용
	// 2=>비밀번호 검색
	@Select("SELECT pm.userId,userName,userPwd,enabled,authority " + "FROM projectMember pm,projectAuthority pa "
	...
	@Select("SELECT pm.userId,userName,userPwd,sex,email,phone,addr1,addr2,enabled,authority "
			+ "FROM projectMember pm,projectAuthority pa " + "WHERE pm.userId=pa.userId " + "AND pm.userId=#{userId}")
	...
	@Update("UPDATE projectMember SET " + "lastlogin=SYSDATE " + "WHERE userId=#{userId}")
